...
def fetch_article_responses(links: list[str]) -> list:
    ...
    article_requests = (grequests.get(x, timeout=10) for x in links)
...
def get_article_date(soup) -> str:
    ...
    return reformat_date(soup.find("span", class_="date"))
...
def get_all_links_from_all_topics() -> list[dict]:
    ...
    if isinstance(topic_links, str):
        return []
    print(f"fetched all topics: {len(topic_links)}")
    responses = fetch_article_responses(topic_links)
    print(f"fetched all responses: {len(responses)}")
    all_article_links = []
    for i, response in enumerate(responses):
        if response is None:
            ...
            print(f"extracted {len(responses)} links from {topic_links[i]}")
...
def scrape_democracy_now():
    ...
    results = parse_all_links(all_links)
    print(f"Extracted title and content from {len(all_links)} articles")
    return results
